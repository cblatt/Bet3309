...
export default function LeagueLeaders() {
        ...
        try{
            axios.get("/off/pass_yds").then(res=>{
                ...
                for(i in list){
                    ...
                    let fname = document.createTextNode('Player: '+res.data[i].pf_name+' ')
                    let lname = document.createTextNode(res.data[i].pl_name)
                    let yds = document.createTextNode('Passing Yards: '+res.data[i].pass_yds)
                    l.appendChild(fname)
                    l.appendChild(lname)
                    l1.appendChild(yds)
                    u.appendChild(l)
        ...
        try{
            axios.get("/off/rush_yds").then(res=>{
                ...
                for(i in list){
                    ...
                    let fname = document.createTextNode('Player: '+res.data[i].pf_name+' ')
                    let lname = document.createTextNode(res.data[i].pl_name)
                    let yds = document.createTextNode('Rushing Yards: '+res.data[i].rush_yds)
                    l.appendChild(fname)
                    l.appendChild(lname)
                    l1.appendChild(yds)
                    u.appendChild(l)
        ...
        try{
            axios.get("/off/rec_yds").then(res=>{
                ...
                for(i in list){
                    ...
                    let fname = document.createTextNode('Player: '+res.data[i].pf_name+' ')
                    let lname = document.createTextNode(res.data[i].pl_name)
                    let yds = document.createTextNode('Recieving Yards: '+res.data[i].rec_yds)
                    l.appendChild(fname)
                    l.appendChild(lname)
                    l1.appendChild(yds)
                    u.appendChild(l)
    ...
    return(
        ...
        <div id="pass">
        <button id="pass_yds" onClick ={showPass}>Show Leading Passing Yards</button>
        <ul className="list" id="pass_ydsList"></ul>
        </div>
        <div id="rec">
        <button id="rec_yds" onClick ={showRec}>Show Leading Recieving Yards</button>
        <ul className="list" id="rec_ydsList"></ul>
        </div>
        <div id="rush">
        <button id="rush_yds" onClick ={showRush}>Show Leading Rushing Yards</button>
