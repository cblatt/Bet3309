        ...
        res.json().then((data) => {
            ...
            for(let i=0; i<data.length; i++){
                ...
                contBtn.addEventListener('click', () => {
                    ...
                    contList.appendChild(document.createTextNode(data[i].pass_yds + ' Passing Yards: ' + (data[i].pass_yds/data[i].pass_yards*100).toFixed(2) + '% of Team Total'));
                    contList.appendChild(document.createElement('br'));

                    contList.appendChild(document.createTextNode(data[i].rec_yds + ' Receiving Yards: ' + (data[i].rec_yds/data[i].rec_yards*100).toFixed(2) + '% of Team Total'));
                    contList.appendChild(document.createElement('br'));

                    contList.appendChild(document.createTextNode(data[i].rush_yds + ' Rushing Yards: ' + (data[i].rush_yds/data[i].rush_yards*100).toFixed(2) + '% of Team Total'));
                    contList.appendChild(document.createElement('br'));

                    contList.appendChild(document.createTextNode(data[i].td*6 + ' Points Scored: ' + (data[i].td*6/data[i].points_for*100).toFixed(2) + '% of Team Total'));
                    contList.appendChild(document.createElement('br'));
