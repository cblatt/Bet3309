...
export default function PlayerComparison() {
    ...
    function search(){
        ...
        fetch(`${process.env.REACT_APP_BACKEND_URL}/playerAndTeam/` + input).then((res) =>
					res.json().then((data) => {
						var searchList = document.getElementById("searchList");
						...
						for (let i = 0; i < data.length; i++) {
							...
							contBtn.addEventListener("click", () => {
								...
								contList.appendChild(
									document.createTextNode(
										data[i].pf_name + " " + data[i].pl_name + " (" + data[i].team_name + ")"
									)
								);
								...
								contList.appendChild(
									document.createTextNode(
										data[i].pass_yds +
											" Passing Yards = " +
											((data[i].pass_yds / data[i].pass_yards) * 100).toFixed(2) +
											"% of Team Total (" +
											data[i].pass_yards +
											" Yards)"
									)
								...
								contList.appendChild(
									document.createTextNode(
										data[i].rec_yds +
											" Receiving Yards = " +
											((data[i].rec_yds / data[i].rec_yards) * 100).toFixed(2) +
											"% of Team Total (" +
											data[i].rec_yards +
											" Yards)"
									)
								...
								contList.appendChild(
									document.createTextNode(
										data[i].rush_yds +
											" Rushing Yards = " +
											((data[i].rush_yds / data[i].rush_yards) * 100).toFixed(2) +
											"% of Team Total (" +
											data[i].rush_yards +
											" Yards)"
									)
								...
								contList.appendChild(
									document.createTextNode(
										data[i].td * 6 +
											" Points Scored = " +
											(((data[i].td * 6) / data[i].points_for) * 100).toFixed(2) +
											"% of Team Total (" +
											data[i].points_for +
											" Points)"
									)
